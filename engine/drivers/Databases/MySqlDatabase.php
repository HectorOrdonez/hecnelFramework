 ...
 */
class MySqlDatabase extends \PDO implements DBInterface
{
    /**
     ...
     * dbHost and dbName will turn into the data source name parameter required by the PDO class.
    ...
    {
        if (!is_array($data)) {
            throw new DatabaseException(sprintf(self::WRONG_FORMAT_INSERT));
        }

        // Is a single insert or multiple?
        if (isset($data[0]) and is_array($data[0])) {
            return $this->multipleRecordInsert($table, $data);
        } else {
            return $this->singleRecordInsert($table, $data);
        }
    }

    /**
     * Insert command for single record insertions.
     * @param $table
     * @param $record
     * @return int
     */
    protected function singleRecordInsert($table, $record)
    {
        // 1 - Preparing unbound query
    ...
    /**
     * Makes a singleRecordInsert call for each new record to produce.
     * @param string $table
     * @param array $records
     * @return array
     * @todo Yes, eventually this might get busy for large inserts. Think about it, noob.
     */
    protected function multipleRecordInsert($table, $records)
    {
        $newIds = array();

        foreach ($records as $record) {
            $newIds[] = $this->singleRecordInsert($table, $record);
        }
     ...
     * @param array $conditions
     * @return int|void
     * @todo Update method has to return the amount of affected records.
     * @todo What happens with no conditions, eh?!
     */
    public function update($table, $set, $conditions = array())
    {
        // 1 - Preparing unbound query
        $setString = '';
        $conditionsString = '';
